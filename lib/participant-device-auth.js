...
async function validateParticipantDevice(sql, participantId, deviceToken) {
  ...
  const participantRows = await sql`
    ...
    WHERE id = ${participantId}
      AND scope = 'live'
      AND (
        game_started_at = (SELECT started_at FROM game_state WHERE scope='live')
        OR created_at >= (SELECT started_at FROM game_state WHERE scope='live')
      )
    LIMIT 1
  `;
  ...
  if (participantRows.length > 0) {
    ...
    await sql`
      ...
      WHERE id = ${participant.id}
        AND scope = 'live'
        AND (
          game_started_at IS DISTINCT FROM (SELECT started_at FROM game_state WHERE scope='live')
  ...
  const deviceRows = await sql`
    ...
    ORDER BY
      CASE WHEN game_started_at = (SELECT started_at FROM game_state WHERE scope='live') THEN 0 ELSE 1 END,
