const { migrate } = require('./schema-migrations');
const PARTICIPANTS_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS participants (
    id SERIAL PRIMARY KEY,
    scope TEXT NOT NULL DEFAULT 'live',
    first_name TEXT NOT NULL,
    last_name TEXT NOT NULL,
    city TEXT,
    email TEXT,
    device_token TEXT,
    game_started_at TIMESTAMPTZ,
    last_ip TEXT,
    last_user_agent TEXT,
    current_question_id INTEGER,
    current_question_opened_at TIMESTAMPTZ,
    score INTEGER DEFAULT 0,
    manual_score_adjustment INTEGER DEFAULT 0,
    score_at TIMESTAMPTZ,
    answered_count INTEGER DEFAULT 0,
    last_activity TIMESTAMPTZ,
    created_at TIMESTAMPTZ DEFAULT NOW()
  )
`;

const ANSWERS_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS answers (
    id SERIAL PRIMARY KEY,
    scope TEXT NOT NULL DEFAULT 'live',
    participant_id INTEGER REFERENCES participants(id),
    question_id INTEGER NOT NULL,
    answer TEXT NOT NULL,
    correct BOOLEAN NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
  )
`;

const GAME_STATE_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS game_state (
    id INTEGER PRIMARY KEY DEFAULT 1,
    scope TEXT NOT NULL DEFAULT 'live',
    started_at TIMESTAMPTZ,
    ended_at TIMESTAMPTZ,
    summary_at TIMESTAMPTZ,
    duration_minutes INTEGER DEFAULT 180,
    announcement_text TEXT,
    announcement_updated_at TIMESTAMPTZ,
    results_participants_revealed_at TIMESTAMPTZ,
    results_cities_revealed_at TIMESTAMPTZ,
    technical_pause_active BOOLEAN NOT NULL DEFAULT FALSE,
    technical_pause_text TEXT,
    technical_pause_updated_at TIMESTAMPTZ
  )
`;

const BONUS_REDEMPTIONS_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS bonus_redemptions (
    id SERIAL PRIMARY KEY,
    scope TEXT NOT NULL DEFAULT 'live',
    participant_id INTEGER REFERENCES participants(id),
    bonus_id TEXT NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(participant_id, bonus_id)
  )
`;

const BONUS_CATALOG_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS bonus_catalog (
    id TEXT PRIMARY KEY,
    label TEXT NOT NULL,
    secret TEXT NOT NULL,
    active BOOLEAN NOT NULL DEFAULT TRUE,
    sort_order INTEGER NOT NULL DEFAULT 0,
    updated_at TIMESTAMPTZ DEFAULT NOW()
  )
`;

const QUESTION_OPENS_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS question_opens (
    id SERIAL PRIMARY KEY,
    scope TEXT NOT NULL DEFAULT 'live',
    participant_id INTEGER REFERENCES participants(id),
    question_id INTEGER NOT NULL,
    created_at TIMESTAMPTZ DEFAULT NOW()
  )
`;

const QUESTIONS_CATALOG_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS questions_catalog (
    id INTEGER PRIMARY KEY,
    text TEXT NOT NULL,
    answers JSONB NOT NULL DEFAULT '[]'::jsonb,
    active BOOLEAN NOT NULL DEFAULT TRUE,
    updated_at TIMESTAMPTZ DEFAULT NOW()
  )
`;

const APP_RUNTIME_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS app_runtime (
    id INTEGER PRIMARY KEY DEFAULT 1,
    admin_view_scope TEXT NOT NULL DEFAULT 'live',
    test_mode_enabled BOOLEAN NOT NULL DEFAULT FALSE,
    test_seeded_at TIMESTAMPTZ,
    test_participant_count INTEGER NOT NULL DEFAULT 0
  )
`;

let coreSchemaPromise = null;
let gameStateSchemaPromise = null;
let bonusSchemaPromise = null;
let bonusCatalogSchemaPromise = null;
let questionTrackingSchemaPromise = null;
let questionCatalogSchemaPromise = null;
let appRuntimeSchemaPromise = null;

async function ensureCoreSchema(sql) {
  if (!coreSchemaPromise) {
    coreSchemaPromise = (async () => {
      await migrate(sql, 'core', 1, async (sql) => {
      await sql(PARTICIPANTS_TABLE_SQL);
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS scope TEXT NOT NULL DEFAULT 'live'`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS city TEXT`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS email TEXT`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS device_token TEXT`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS game_started_at TIMESTAMPTZ`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS last_ip TEXT`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS last_user_agent TEXT`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS current_question_id INTEGER`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS current_question_opened_at TIMESTAMPTZ`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS score INTEGER DEFAULT 0`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS manual_score_adjustment INTEGER`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS score_at TIMESTAMPTZ`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS answered_count INTEGER DEFAULT 0`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS last_activity TIMESTAMPTZ`;
      await sql`ALTER TABLE participants ADD COLUMN IF NOT EXISTS created_at TIMESTAMPTZ DEFAULT NOW()`;
      await sql(ANSWERS_TABLE_SQL);
      await sql`ALTER TABLE answers ADD COLUMN IF NOT EXISTS scope TEXT NOT NULL DEFAULT 'live'`;
      await sql`UPDATE participants SET scope = 'live' WHERE scope IS NULL`;
      await sql`UPDATE answers SET scope = 'live' WHERE scope IS NULL`;
      await sql`
        WITH duplicate_answers AS (
          SELECT id
          FROM (
            SELECT
              id,
              ROW_NUMBER() OVER (
                PARTITION BY participant_id, question_id
                ORDER BY created_at ASC, id ASC
              ) AS row_num
            FROM answers
            WHERE participant_id IS NOT NULL
          ) ranked
          WHERE row_num > 1
        )
        DELETE FROM answers
        WHERE id IN (SELECT id FROM duplicate_answers)
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_participants_device_token ON participants(device_token)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_participants_scope_game_started_device ON participants(scope, game_started_at, device_token)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_participants_name_city ON participants(first_name, last_name, city)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_participants_email ON participants(email)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_participants_last_activity ON participants(last_activity DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_participants_scope_last_activity ON participants(scope, last_activity DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_participants_scope_score_activity ON participants(scope, score DESC, answered_count DESC, last_activity ASC, id ASC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_answers_participant_created_at ON answers(participant_id, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_answers_question_created_at ON answers(question_id, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_answers_scope_question_created_at ON answers(scope, question_id, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_answers_scope_created_at ON answers(scope, created_at DESC)`;
      await sql`
        CREATE UNIQUE INDEX IF NOT EXISTS idx_answers_participant_question_unique
        ON answers(participant_id, question_id)
      `;
      });
    })().catch((error) => {
      coreSchemaPromise = null;
      throw error;
    });
  }

  await coreSchemaPromise;
}

async function ensureGameStateSchema(sql) {
  if (!gameStateSchemaPromise) {
    gameStateSchemaPromise = (async () => {
      await ensureCoreSchema(sql);
      await migrate(sql, 'game-state', 1, async (sql) => {
      await sql(GAME_STATE_TABLE_SQL);
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS scope TEXT NOT NULL DEFAULT 'live'`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS round_started_at TIMESTAMPTZ`;
      await sql`UPDATE game_state g SET round_started_at = COALESCE(g.started_at, (SELECT MAX(p.game_started_at) FROM participants p WHERE p.scope=g.scope)) WHERE g.round_started_at IS NULL`;
      await sql`UPDATE participants p SET game_started_at=g.round_started_at FROM game_state g WHERE p.scope=g.scope AND p.game_started_at IS NULL AND p.created_at>=g.round_started_at`;
      await sql`WITH ranked AS (SELECT id, ROW_NUMBER() OVER (PARTITION BY scope, game_started_at, device_token ORDER BY id DESC) AS n FROM participants WHERE device_token IS NOT NULL AND game_started_at IS NOT NULL) UPDATE participants SET device_token=NULL WHERE id IN (SELECT id FROM ranked WHERE n>1)`;
      await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_participants_round_device_unique ON participants(scope,game_started_at,device_token) WHERE device_token IS NOT NULL`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS ended_at TIMESTAMPTZ`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS summary_at TIMESTAMPTZ`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS duration_minutes INTEGER DEFAULT 180`;
      await sql`ALTER TABLE game_state ALTER COLUMN duration_minutes SET DEFAULT 180`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS announcement_text TEXT`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS announcement_updated_at TIMESTAMPTZ`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS results_participants_revealed_at TIMESTAMPTZ`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS results_cities_revealed_at TIMESTAMPTZ`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS technical_pause_active BOOLEAN NOT NULL DEFAULT FALSE`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS technical_pause_text TEXT`;
      await sql`ALTER TABLE game_state ADD COLUMN IF NOT EXISTS technical_pause_updated_at TIMESTAMPTZ`;
      await sql`UPDATE game_state SET scope = 'live' WHERE scope IS NULL`;
      await sql`CREATE UNIQUE INDEX IF NOT EXISTS idx_game_state_scope_unique ON game_state(scope)`;
      });
    })().catch((error) => {
      gameStateSchemaPromise = null;
      throw error;
    });
  }

  await gameStateSchemaPromise;
}

async function ensureBonusSchema(sql) {
  if (!bonusSchemaPromise) {
    bonusSchemaPromise = (async () => {
      await ensureGameStateSchema(sql);
      await migrate(sql, 'bonus-redemptions', 1, async (sql) => {
      await sql(BONUS_REDEMPTIONS_TABLE_SQL);
      await sql`ALTER TABLE bonus_redemptions ADD COLUMN IF NOT EXISTS scope TEXT NOT NULL DEFAULT 'live'`;
      await sql`UPDATE bonus_redemptions SET scope = 'live' WHERE scope IS NULL`;
      await sql`
        WITH duplicate_bonus_redemptions AS (
          SELECT id
          FROM (
            SELECT
              id,
              ROW_NUMBER() OVER (
                PARTITION BY participant_id, bonus_id
                ORDER BY created_at ASC, id ASC
              ) AS row_num
            FROM bonus_redemptions
            WHERE participant_id IS NOT NULL
          ) ranked
          WHERE row_num > 1
        )
        DELETE FROM bonus_redemptions
        WHERE id IN (SELECT id FROM duplicate_bonus_redemptions)
      `;
      await sql`CREATE INDEX IF NOT EXISTS idx_bonus_redemptions_participant_created_at ON bonus_redemptions(participant_id, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_bonus_redemptions_bonus_created_at ON bonus_redemptions(bonus_id, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_bonus_redemptions_scope_bonus_created_at ON bonus_redemptions(scope, bonus_id, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_bonus_redemptions_scope_created_at ON bonus_redemptions(scope, created_at DESC)`;
      await sql`
        CREATE UNIQUE INDEX IF NOT EXISTS idx_bonus_redemptions_participant_bonus_unique
        ON bonus_redemptions(participant_id, bonus_id)
      `;
      });
    })().catch((error) => {
      bonusSchemaPromise = null;
      throw error;
    });
  }

  await bonusSchemaPromise;
}

async function ensureBonusCatalogSchema(sql) {
  if (!bonusCatalogSchemaPromise) {
    bonusCatalogSchemaPromise = (async () => {
      const { BONUS_CODES } = require('./bonus-codes-data');

      await ensureBonusSchema(sql);
      await migrate(sql, 'bonus-catalog', 1, async (sql) => {
      await sql(BONUS_CATALOG_TABLE_SQL);
      await sql`ALTER TABLE bonus_catalog ADD COLUMN IF NOT EXISTS label TEXT`;
      await sql`ALTER TABLE bonus_catalog ADD COLUMN IF NOT EXISTS secret TEXT`;
      await sql`ALTER TABLE bonus_catalog ADD COLUMN IF NOT EXISTS active BOOLEAN NOT NULL DEFAULT TRUE`;
      await sql`ALTER TABLE bonus_catalog ADD COLUMN IF NOT EXISTS technical_blocked BOOLEAN NOT NULL DEFAULT FALSE`;
      await sql`ALTER TABLE bonus_catalog ADD COLUMN IF NOT EXISTS sort_order INTEGER NOT NULL DEFAULT 0`;
      await sql`ALTER TABLE bonus_catalog ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`;

      for (let index = 0; index < BONUS_CODES.length; index += 1) {
        const bonus = BONUS_CODES[index];
        await sql`
          INSERT INTO bonus_catalog (id, label, secret, active, sort_order)
          VALUES (${bonus.id}, ${bonus.label}, ${bonus.secret}, TRUE, ${index + 1})
          ON CONFLICT (id) DO NOTHING
        `;
      }
      });
    })().catch((error) => {
      bonusCatalogSchemaPromise = null;
      throw error;
    });
  }

  await bonusCatalogSchemaPromise;
}

async function ensureQuestionTrackingSchema(sql) {
  if (!questionTrackingSchemaPromise) {
    questionTrackingSchemaPromise = (async () => {
      await ensureGameStateSchema(sql);
      await migrate(sql, 'question-tracking', 1, async (sql) => {
      await sql(QUESTION_OPENS_TABLE_SQL);
      await sql`ALTER TABLE question_opens ADD COLUMN IF NOT EXISTS scope TEXT NOT NULL DEFAULT 'live'`;
      await sql`UPDATE question_opens SET scope = 'live' WHERE scope IS NULL`;
      await sql`CREATE INDEX IF NOT EXISTS idx_question_opens_created_at ON question_opens(created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_question_opens_question_created_at ON question_opens(question_id, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_question_opens_participant_created_at ON question_opens(participant_id, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_question_opens_scope_created_at ON question_opens(scope, created_at DESC)`;
      await sql`CREATE INDEX IF NOT EXISTS idx_question_opens_scope_question_created_at ON question_opens(scope, question_id, created_at DESC)`;
      });
    })().catch((error) => {
      questionTrackingSchemaPromise = null;
      throw error;
    });
  }

  await questionTrackingSchemaPromise;
}

async function ensureQuestionCatalogSchema(sql) {
  if (!questionCatalogSchemaPromise) {
    questionCatalogSchemaPromise = (async () => {
      const { QUESTIONS } = require('./questions-data');

      await ensureGameStateSchema(sql);
      await migrate(sql, 'question-catalog', 1, async (sql) => {
      await sql(QUESTIONS_CATALOG_TABLE_SQL);
      await sql`ALTER TABLE questions_catalog ADD COLUMN IF NOT EXISTS text TEXT`;
      await sql`ALTER TABLE questions_catalog ADD COLUMN IF NOT EXISTS answers JSONB NOT NULL DEFAULT '[]'::jsonb`;
      await sql`ALTER TABLE questions_catalog ADD COLUMN IF NOT EXISTS active BOOLEAN NOT NULL DEFAULT TRUE`;
      await sql`ALTER TABLE questions_catalog ADD COLUMN IF NOT EXISTS technical_blocked BOOLEAN NOT NULL DEFAULT FALSE`;
      await sql`ALTER TABLE questions_catalog ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`;

      for (const question of QUESTIONS) {
        await sql`
          INSERT INTO questions_catalog (id, text, answers)
          VALUES (${question.id}, ${question.text}, ${JSON.stringify(question.answers || [])}::jsonb)
          ON CONFLICT (id) DO NOTHING
        `;
      }
      });
    })().catch((error) => {
      questionCatalogSchemaPromise = null;
      throw error;
    });
  }

  await questionCatalogSchemaPromise;
}

async function ensureAppRuntimeSchema(sql) {
  if (!appRuntimeSchemaPromise) {
    appRuntimeSchemaPromise = (async () => {
      await ensureGameStateSchema(sql);
      await migrate(sql, 'app-runtime', 1, async (sql) => {
      await sql(APP_RUNTIME_TABLE_SQL);
      await sql`ALTER TABLE app_runtime ADD COLUMN IF NOT EXISTS admin_view_scope TEXT NOT NULL DEFAULT 'live'`;
      await sql`ALTER TABLE app_runtime ADD COLUMN IF NOT EXISTS test_mode_enabled BOOLEAN NOT NULL DEFAULT FALSE`;
      await sql`ALTER TABLE app_runtime ADD COLUMN IF NOT EXISTS test_seeded_at TIMESTAMPTZ`;
      await sql`ALTER TABLE app_runtime ADD COLUMN IF NOT EXISTS test_participant_count INTEGER NOT NULL DEFAULT 0`;
      await sql`
        INSERT INTO app_runtime (id, admin_view_scope, test_mode_enabled, test_seeded_at, test_participant_count)
        VALUES (1, 'live', FALSE, NULL, 0)
        ON CONFLICT (id) DO NOTHING
      `;
      });
    })().catch((error) => {
      appRuntimeSchemaPromise = null;
      throw error;
    });
  }

  await appRuntimeSchemaPromise;
}

let scoringSchemaPromise = null;
async function ensureScoringSchema(sql) {
  if (!scoringSchemaPromise) scoringSchemaPromise = (async () => {
    await ensureBonusCatalogSchema(sql);
    await ensureQuestionCatalogSchema(sql);
    await migrate(sql, 'scoring', 1, async (sql) => {
    await sql`UPDATE participants p SET manual_score_adjustment=COALESCE(p.score,0)
      -(SELECT COUNT(*)::int FROM answers a JOIN questions_catalog q ON q.id=a.question_id AND q.active=TRUE WHERE a.participant_id=p.id AND a.scope=p.scope AND a.correct=TRUE)
      -(SELECT COUNT(*)::int FROM bonus_redemptions b WHERE b.participant_id=p.id AND b.scope=p.scope)
      WHERE p.manual_score_adjustment IS NULL`;
    await sql`ALTER TABLE participants ALTER COLUMN manual_score_adjustment SET DEFAULT 0`;
    await sql`UPDATE participants p SET score_at=GREATEST(p.created_at,(SELECT MAX(created_at) FROM answers WHERE participant_id=p.id),(SELECT MAX(created_at) FROM bonus_redemptions WHERE participant_id=p.id)) WHERE score_at IS NULL`;
    });
    await migrate(sql, 'score-time-ranking', 1, async q => {
      q`UPDATE participants p SET score_at=GREATEST(p.created_at,
        (SELECT MAX(a.created_at) FROM answers a JOIN questions_catalog c ON c.id=a.question_id AND c.active WHERE a.participant_id=p.id AND a.correct),
        (SELECT MAX(b.created_at) FROM bonus_redemptions b WHERE b.participant_id=p.id))
        WHERE COALESCE(p.manual_score_adjustment,0)=0`;
    });
  })().catch(error => { scoringSchemaPromise=null; throw error; });
  await scoringSchemaPromise;
}
module.exports = {
  ensureScoringSchema,
  ensureCoreSchema,
  ensureGameStateSchema,
  ensureBonusSchema,
  ensureBonusCatalogSchema,
  ensureQuestionTrackingSchema,
  ensureQuestionCatalogSchema,
  ensureAppRuntimeSchema
};
