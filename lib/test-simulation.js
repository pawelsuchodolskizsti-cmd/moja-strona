...
const SIMULATION_DURATION_MINUTES = 180;
...
function clearTestQueries(sql) {
  return [require('./round-archives').archiveQuery(sql,TEST_SCOPE,'test-clear'),sql`DELETE FROM answers WHERE scope=${TEST_SCOPE}`,sql`DELETE FROM bonus_redemptions WHERE scope=${TEST_SCOPE}`,
    sql`DELETE FROM question_opens WHERE scope=${TEST_SCOPE}`,sql`DELETE FROM participants WHERE scope=${TEST_SCOPE}`,sql`DELETE FROM game_state WHERE scope=${TEST_SCOPE}`];
}

async function readTestCounts(sql) {
  await ensureSimulationSchema(sql);await require('./operations-schema').ensureOperationsSchema(sql);
...
async function materializeTestProgress(sql, options = {}) {
  ...
  const gameRows = await sql`
    SELECT started_at, ended_at, duration_minutes, technical_pause_active
    FROM game_state
  ...
  if (!gameState?.started_at || gameState.ended_at || gameState.technical_pause_active) {
  ...
  const [participants, questions, bonuses, existingAnswers, existingBonuses] = await Promise.all([
    sql`
      ...
      FROM participants
      WHERE scope = ${TEST_SCOPE} AND game_started_at=(SELECT started_at FROM game_state WHERE scope=${TEST_SCOPE})
      ORDER BY id ASC
    `,
  ...
  for(const answer of dueAnswers.slice(0, Number(options.maxAnswers || ANSWER_BATCH_LIMIT))) {
    queries.push(sql`
      WITH inserted AS (
        ...
        SELECT ${TEST_SCOPE},p.id,q.id,${answer.answer},
          EXISTS(SELECT 1 FROM jsonb_array_elements_text(q.answers) accepted(value)
            WHERE regexp_replace(normalize(lower(btrim(accepted.value)),NFD),'[̀-ͯ]','','g')=regexp_replace(normalize(lower(btrim(${answer.answer}::text)),NFD),'[̀-ͯ]','','g')),
          ${answer.createdAt}
        FROM participants p JOIN game_state g ON g.scope=p.scope AND g.started_at=p.game_started_at
        JOIN questions_catalog q ON q.id=${answer.questionId} AND q.active=TRUE
        WHERE p.id=${answer.participantId} AND p.scope=${TEST_SCOPE} AND NOT g.technical_pause_active
        ON CONFLICT(participant_id,question_id) DO NOTHING RETURNING participant_id,question_id
      ) INSERT INTO question_opens(scope,participant_id,question_id,created_at)
        SELECT ${TEST_SCOPE},participant_id,question_id,${answer.openCreatedAt} FROM inserted RETURNING id
    `);
  }
  const answerQueryCount=queries.length;
  for(const bonus of dueBonuses.slice(0, Number(options.maxBonuses || BONUS_BATCH_LIMIT))) {
    queries.push(sql`
      ...
      SELECT ${TEST_SCOPE},p.id,b.id,${bonus.createdAt}
      FROM participants p JOIN game_state g ON g.scope=p.scope AND g.started_at=p.game_started_at
      JOIN bonus_catalog b ON b.id=${bonus.bonusId} AND b.active=TRUE
      WHERE p.id=${bonus.participantId} AND p.scope=${TEST_SCOPE} AND NOT g.technical_pause_active
      ON CONFLICT(participant_id,bonus_id) DO NOTHING RETURNING id
    `);
  }
  if(!queries.length) return {insertedAnswers:0,insertedBonuses:0};
  const results=await transaction(sql,[...queries,scoreQuery(sql,null,TEST_SCOPE)],{exclusive:true});
  return {insertedAnswers:results.slice(0,answerQueryCount).reduce((n,r)=>n+r.length,0),
    insertedBonuses:results.slice(answerQueryCount,-1).reduce((n,r)=>n+r.length,0)};
}

async function seedLargeTestSimulation(sql) {
  await ensureSimulationSchema(sql);await require('./operations-schema').ensureOperationsSchema(sql);
  const institutions=await require('./institutions').getInstitutions(sql,true);

  const now = new Date();
  const startedAt = new Date(now.getTime() - SIMULATION_INITIAL_OFFSET_MS);
  const queries=clearTestQueries(sql);
  ...
  queries.push(sql`UPDATE participants SET game_started_at=${startedAt} WHERE scope=${TEST_SCOPE}`);
  queries.push(sql`UPDATE game_state SET round_started_at=${startedAt} WHERE scope=${TEST_SCOPE}`);
  queries.push(sql`
    UPDATE app_runtime
    SET
  ...
  await transaction(sql,queries,{exclusive:true});
...
async function clearTestMode(sql) {
  await ensureSimulationSchema(sql);await require('./operations-schema').ensureOperationsSchema(sql);
  await transaction(sql,[...clearTestQueries(sql),sql`
    UPDATE app_runtime
    SET
