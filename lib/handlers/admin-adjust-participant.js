  ...
  try {
    ...
    if (action === 'remove-point') {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, message: 'Odjęto 1 punkt.' })
      };
    }
    ...
    if (action === 'add-bonus') {
      ...
      if (existing.length > 0) {
        return {
          statusCode: 200,
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ok: false, error: 'Ten bonus jest już przypisany do uczestnika.' })
        };
      }
    ...
    if (action === 'remove-bonus') {
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, message: `Usunięto bonus ${bonus.label}.` })
      };
    }
