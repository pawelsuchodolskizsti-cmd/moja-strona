...
function getPhase(gameState) {
  ...
  const summary=gameState?.summary_at?new Date(gameState.summary_at).getTime():null;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const { scope, runtime } = await resolveScope(sql, event?.queryStringParameters?.scope || 'live');
    if (scope === TEST_SCOPE) {
    ...
    const [gameRows, participantRows, answerCountRows, bonusCountRows] = await Promise.all([
      ...
      sql`
        SELECT
          ...
          city,institution_id AS "institutionId",
          score,
    ...
    decorate(participantRows,institutions);
    ...
    const leaderboard = participantRows.map((participant, index) => ({
      ...
      city: participant.city || '',institutionId:participant.institutionId||null,
    ...
    const cityStats = require('../../public/game-ranking').institutionStats(participantRows);
    ...
    const personsVisible=!suspense&&(!final||Boolean(gameState.results_participants_revealed_at));
    const citiesVisible=!suspense&&(!final||Boolean(gameState.results_cities_revealed_at));
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        leaderboard: personsVisible?leaderboard.map(p=>citiesVisible?p:{...p,city:'',institutionId:null}):[]
      })
    };
  } catch (error) {
    console.error(error);
