...
function getPhase(gameState) {
  ...
  if(start && Date.now()<start+Number(gameState.duration_minutes||10)*60000) return gameState.technical_pause_active?'paused':'active';
  const summary=gameState?.summary_at?new Date(gameState.summary_at).getTime():start?start+Number(gameState.duration_minutes||10)*60000+300000:null;
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const remaining=gameState.started_at?new Date(gameState.started_at).getTime()+Number(gameState.duration_minutes||10)*60000-Date.now():null;
    ...
    const naturalEnd=gameState.started_at?new Date(new Date(gameState.started_at).getTime()+Number(gameState.duration_minutes||10)*60000):null;
    return {
      statusCode: 200,
      headers: {
