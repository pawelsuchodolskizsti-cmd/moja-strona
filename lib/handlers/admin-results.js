...
exports.handler = async (event) => {
  ...
  const answers = await sql`
    SELECT
      ...
      p.city AS "participantCity",p.institution_id AS "institutionId",
    ...
    WHERE a.scope = ${scope}
      AND p.scope = ${scope} AND p.game_started_at IS NOT DISTINCT FROM (SELECT round_started_at FROM game_state WHERE scope=${scope})
    ORDER BY a.created_at DESC
  `;

  decorate(answers,institutions,'participantCity');
  const answersWithText = answers.map(a => ({
    ...a,
    questionText: questionMap.get(a.questionId)?.text || null
  }));
  ...
  const bonusRedemptions = await sql`
    ...
    WHERE scope = ${scope} AND participant_id IN (SELECT id FROM participants WHERE scope=${scope} AND game_started_at IS NOT DISTINCT FROM (SELECT round_started_at FROM game_state WHERE scope=${scope}))
    ORDER BY created_at DESC
  `;

  const bonusesWithLabels = bonusRedemptions.map((bonus) => ({
    ...bonus,
    bonusLabel: bonusMap.get(bonus.bonusId)?.label || bonus.bonusId
  }));
