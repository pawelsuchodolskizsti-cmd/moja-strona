...
exports.handler = async (event) => {
  ...
  try {
    ...
    const [inserted] = await transaction(sql,[sql`
      ...
      FROM participants p JOIN game_state g ON g.scope=p.scope AND g.started_at=p.game_started_at JOIN questions_catalog q ON q.id=${questionId} AND q.active=TRUE
