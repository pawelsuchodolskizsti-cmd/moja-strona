...
exports.handler = async (event) => {
  ...
  let activeParticipantId = Number(participantId);
  ...
  try {
    ...
    const inserted = await sql`
      ...
      VALUES ('live', ${activeParticipantId}, ${questionId}, ${answer.trim()}, ${correct})
    ...
    if (inserted.length === 0) {
      const duplicates = await sql`
        SELECT id, answer
        FROM answers
        WHERE participant_id = ${activeParticipantId} AND question_id = ${questionId} AND scope = 'live'
        LIMIT 1
      `;
      ...
      return {
        ...
        body: JSON.stringify({
          ...
          answer: duplicates[0]?.answer || answer.trim(),
          message: 'Odpowiedź została już wcześniej wysłana.',
          stats
        })
      };
    }
