...
const DEFAULT_TECHNICAL_PAUSE_TEXT = 'Za chwilę wracamy. Trwa krótka przerwa techniczna.';
...
function getDefaultState(scope = LIVE_SCOPE) {
  return {
    ...
    technical_pause_text: null,
    technical_pause_updated_at: null
  };
}
  ...
  if (event.httpMethod === 'POST') {
    ...
    if (action === 'start') {
      ...
      await sql`
        ...
        VALUES (${stateId}, ${scope}, NOW(), NULL, NULL, 10, NULL, NULL, NULL, NULL, FALSE, NULL, NULL)
    ...
    if (action === 'stop') {
      ...
      await sql`
        ...
        VALUES (${stateId}, ${scope}, NULL, NOW(), NOW() + INTERVAL '5 minutes', 10, NULL, NULL, NULL, NULL, FALSE, NULL, NULL)
        ON CONFLICT (scope) DO UPDATE
        SET
          id = EXCLUDED.id,
          started_at = NULL,
    ...
    if (action === 'set-announcement') {
      ...
      await sql`
        ...
        VALUES (${stateId}, ${scope}, NULL, NULL, NULL, 10, NULL, NULL, NULL, NULL, FALSE, NULL, NULL)
    ...
    if (action === 'reveal-participants' || action === 'reveal-cities' || action === 'lock-results') {
      await sql`
        INSERT INTO game_state (
          id,
          scope,
      ...
      if (action === 'reveal-participants') {
        await sql`
          UPDATE game_state
          SET results_participants_revealed_at = COALESCE(results_participants_revealed_at, NOW())
          WHERE scope = ${scope}
        `;
      } else if (action === 'reveal-cities') {
        await sql`
          UPDATE game_state
          SET
            results_participants_revealed_at = COALESCE(results_participants_revealed_at, NOW()),
            results_cities_revealed_at = COALESCE(results_cities_revealed_at, NOW())
          WHERE scope = ${scope}
        `;
      } else {
        await sql`
      ...
      return {
        statusCode: 200,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ok: true, action, ...state })
      };
    }

    if (action === 'technical-pause') {
      const pauseActive = payload.active !== false;
      const normalizedMessage = String(message || '').trim() || DEFAULT_TECHNICAL_PAUSE_TEXT;
      ...
      if (pauseActive) {
        await sql`
          UPDATE game_state
          SET
            technical_pause_active = TRUE,
            technical_pause_text = ${normalizedMessage},
            technical_pause_updated_at = NOW()
          WHERE scope = ${scope}
        `;
      } else {
        await sql`
      ...
      return {
        ...
        body: JSON.stringify({
          ok: true,
          technicalPauseActive: pauseActive,
          ...state
        })
      };
    }
