...
exports.handler=withApi('game-state',async event=>{
  ...
  await ensureScoringSchema(sql);await ensureQuestionTrackingSchema(sql);await ensureAppRuntimeSchema(sql);await ensureOperationsSchema(sql);
  ...
  if(action==='start') {
    ...
    queries=[sql`UPDATE game_state SET summary_at=CASE WHEN ${action}='show-thanks' THEN NOW() ELSE NULL END
