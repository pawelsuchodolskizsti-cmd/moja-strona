  ...
  try {
    ...
    const [existing, participantRows, bonusRows] = await Promise.all([
      sql`
        SELECT answer
        FROM answers
        WHERE participant_id = ${activeParticipantId} AND question_id = ${qNum} AND scope = 'live'
        LIMIT 1
      `,
      sql`
        SELECT answered_count AS "answers"
        FROM participants
        WHERE id = ${activeParticipantId} AND scope = 'live'
        LIMIT 1
      `,
      sql`
        SELECT COUNT(*)::int AS "bonuses"
        FROM bonus_redemptions
        WHERE participant_id = ${activeParticipantId} AND scope = 'live'
      `
    ]);
    ...
    if (shouldTrackOpen) {
      await sql`
        ...
        VALUES ('live', ${activeParticipantId}, ${qNum})
      `;
