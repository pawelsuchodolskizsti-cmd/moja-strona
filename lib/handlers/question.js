...
exports.handler = async (event) => {
  ...
  try {
    ...
    const blocked = await checkContentBlocked(sql, 'question', qNum);
    ...
    if (shouldTrackOpen) {
      const [opened] = await transaction(sql, [sql`
        ...
        JOIN questions_catalog q ON q.id=${qNum} AND q.active=TRUE AND NOT q.technical_blocked
      ...
      `,sql`
        ...
        FROM game_state g WHERE p.id=${pid} AND p.scope='live' AND p.device_token=${deviceToken.trim()}
          ...
          AND EXISTS(SELECT 1 FROM questions_catalog q WHERE q.id=${qNum} AND q.active=TRUE AND NOT q.technical_blocked)
      `],{participantId:pid});
      if(!opened.length) {
        const blockedNow = await checkContentBlocked(sql, 'question', qNum);
