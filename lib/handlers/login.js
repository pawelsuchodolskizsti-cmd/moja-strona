...
exports.handler=withApi('login',async event=>{
  ...
  const firstName=upper(body.firstName),lastName=upper(body.lastName),city=upper(body.city),email=String(body.email||'').trim().toLowerCase(),token=body.deviceToken.trim();
  if(!firstName||!lastName||!city||!email)return json(400,{error:'Podaj imię, nazwisko, miasto i adres e-mail.'});
  ...
  const results=await transaction(sql,[
    ...
    sql`UPDATE participants p SET device_token=${token},last_activity=NOW(),last_ip=${ip},last_user_agent=${agent}
      WHERE p.id=(SELECT r.id FROM participants r JOIN game_state g ON g.scope=r.scope AND r.game_started_at=g.started_at
        WHERE r.scope='live' AND g.started_at IS NOT NULL AND NOW()<g.started_at+g.duration_minutes*INTERVAL '1 minute' AND NOT g.technical_pause_active
          AND r.device_token IS NULL AND r.first_name=${firstName} AND r.last_name=${lastName} AND r.city=${city} AND r.email=${email}
    ...
    sql`INSERT INTO participants(scope,first_name,last_name,city,email,device_token,game_started_at,last_ip,last_user_agent,last_activity,score_at)
      SELECT 'live',${firstName},${lastName},${city},${email},${token},g.started_at,${ip},${agent},NOW(),NOW()
      FROM game_state g WHERE g.scope='live' AND g.started_at IS NOT NULL AND NOW()<g.started_at+g.duration_minutes*INTERVAL '1 minute' AND NOT g.technical_pause_active
    ...
    sql`UPDATE participants p SET last_activity=NOW(),last_ip=${ip},last_user_agent=${agent},current_question_id=NULL,current_question_opened_at=NULL
      FROM game_state g WHERE g.scope='live' AND p.scope=g.scope AND p.game_started_at=g.started_at AND p.device_token=${token}
        AND p.first_name=${firstName} AND p.last_name=${lastName} AND p.city=${city} AND p.email=${email}`,
    sql`SELECT p.id,p.first_name AS "firstName",p.last_name AS "lastName",p.city,p.email,p.game_started_at AS "gameStartedAt"
  ...
  if(p.firstName!==firstName||p.lastName!==lastName||p.city!==city||p.email!==email)return json(409,{error:'To urządzenie jest już przypisane do innego uczestnika. Skontaktuj się z administratorem.'});
  return json(200,{id:p.id,firstName:p.firstName,lastName:p.lastName,gameStartedAt:p.gameStartedAt,reused:!results[1].length});
});
