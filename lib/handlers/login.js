...
const {ensureCommunicationSchema,validateChoice,consentQuery}=require('../communication-consent');
exports.handler=withApi('login',async event=>{
  ...
  const consentError=validateChoice(body);if(consentError)return json(400,{error:consentError});
  ...
  if(body.communicationConsent===true)await ensureCommunicationSchema(sql);
  const queries=[
  ...
  if(body.communicationConsent===true)queries.push(consentQuery(sql,{firstName,lastName,institutionId,email,deviceToken:token,body}));
  const results=await transaction(sql,queries,{deviceToken:token});
  const p=results[3][0];
