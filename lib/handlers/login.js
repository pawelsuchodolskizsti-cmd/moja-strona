...
exports.handler = async (event) => {
  ...
  if (acceptedRules !== true) {
    return { statusCode: 400, body: JSON.stringify({ error: 'Aby dołączyć do gry, zaakceptuj regulamin.' }) };
  }
  ...
  try {
    ...
    if (!isActive) {
      return {
        ...
        body: JSON.stringify({ error: 'Gra jest już zakończona lub jeszcze nie wystartowała.' })
      };
    }
    ...
    if (normalizedDeviceToken) {
      await sql`
        ...
        WHERE device_token = ${normalizedDeviceToken}
          AND scope = 'live'
          AND (
            game_started_at IS NULL
            OR game_started_at <> ${gameStartedAt}
          )
      `;
    ...
    if (releasedParticipants.length > 1 && !normalizedCity) {
      return {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          error: 'To konto czeka na ponowne przypisanie. Zaloguj się najpierw na ekranie głównym z imieniem, nazwiskiem i miastem.'
        })
      };
    }
    ...
    const rows = await sql`
      INSERT INTO participants (scope, first_name, last_name, city, email, device_token, game_started_at, last_ip, last_user_agent, last_activity)
      VALUES (
        'live',
        ${normalizedFirstName},
        ${normalizedLastName},
    ...
    return {
      statusCode: 200,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: rows[0].id, firstName: normalizedFirstName, lastName: normalizedLastName, gameStartedAt })
    };
  } catch (err) {
    console.error(err);
