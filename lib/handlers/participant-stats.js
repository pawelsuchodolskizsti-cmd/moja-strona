  ...
  try {
    ...
    const participantRows = await sql`
      SELECT
        ...
        (SELECT COUNT(*)::int FROM bonus_redemptions WHERE participant_id=${participantId} AND scope='live') AS bonuses
      FROM participants
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        bonuses: participantRows[0]?.bonuses || 0
      })
    };
  } catch (err) {
    console.error(err);
...
exports.handler = withApi('participant-stats', exports.handler);
