...
async function fetchQrBuffer(data,size,format) {
  ...
  const options={width,margin:4,errorCorrectionLevel:'M'};
  if(format==='svg')return {buffer:Buffer.from(await QRCode.toString(data,{...options,type:'svg'})),contentType:'image/svg+xml'};
  const png=await QRCode.toBuffer(data,{...options,type:'png'});
  if(format==='jpg'){const raw=PNG.sync.read(png);return {buffer:jpeg.encode(raw,90).data,contentType:'image/jpeg'};}
...
async function handleSingleQr(event) {
  ...
  if(data.length>2000)return {statusCode:400,body:JSON.stringify({error:'Adres QR jest zbyt długi.'})};
  ...
  try {
    ...
    return {
      statusCode: 502,
      headers: { 'Content-Type': 'application/json; charset=utf-8' },
      body: JSON.stringify({ error: 'Nie udało się utworzyć QR. Sprawdź długość i poprawność adresu.' })
    };
  }
}
...
async function handleZipBundle(event) {
  ...
  if (!items.length || items.length>312) {
    return {
      statusCode: 400,
      headers: { 'Content-Type': 'application/json; charset=utf-8' },
...
exports.handler = withApi('qr-proxy', exports.handler);
