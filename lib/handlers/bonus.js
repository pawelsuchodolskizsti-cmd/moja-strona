...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      const blocked = await checkContentBlocked(sql, 'bonus', bonusId);
    ...
    if (event.httpMethod === 'POST') {
      ...
      const [inserted] = await transaction(sql,[sql`
        ...
        JOIN bonus_catalog b ON b.id=${bonusId} AND b.active=TRUE AND NOT b.technical_blocked
      ...
      if (inserted.length === 0) {
        const blockedNow = await checkContentBlocked(sql, 'bonus', bonusId);
