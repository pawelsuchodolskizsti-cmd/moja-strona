...
function buildSimulationPlan(participantCount, questions, bonusCatalog) {
  const now = Date.now();
  const questionCount = questions.length || 30;
  const bonusCount = bonusCatalog.length || 10;
  ...
  return Array.from({ length: participantCount }, (_, index) => {
    ...
    const bonuses = bonusCatalog.slice(0, bonusTarget).map((bonus, bonusIndex) => ({
      bonusId: bonus.id,
...
exports.handler = async (event) => {
  ...
  if (action === 'enable') {
    ...
    if (!counts.participants) {
      return {
        statusCode: 400,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ error: 'Najpierw wygeneruj dane testowe do podglądu.' })
      };
    }
  ...
  if (action === 'seed') {
    ...
    const [questionCatalog, bonusCatalog] = await Promise.all([
      getQuestionCatalog(sql),
      getBonusCatalog(sql)
    ]);
    const plan = buildSimulationPlan(participantCount, questionCatalog, bonusCatalog);
    const now = new Date();
    ...
    await sql`
      ...
      VALUES (
        ...
        NULL,
        10,
        'Tryb testowy jest aktywny. Ranking i tablica wyników pokazują dane symulacyjne zapisane w bazie.',
        ${now}
      )
    ...
    return {
      ...
      body: JSON.stringify({
        ok: true,
        message: `Wygenerowano ${participantCount} uczestników testowych i włączono podgląd danych testowych.`,
