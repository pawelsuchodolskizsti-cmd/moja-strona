...
function withApi(name, handler) {
  const adminEndpoints = ['admin-results','admin-question-catalog','admin-adjust-participant','test-mode','qr-proxy'];
  const methods = {
    'admin-session':['GET','POST','DELETE'], 'game-state':['GET','POST'], 'login':['POST'],
  ...
  return async event => {
    try {
      ...
      if (method === 'POST') {
        ...
        for (const key of ['participantId','questionId']) if (body[key] !== undefined && (!Number.isSafeInteger(Number(body[key])) || Number(body[key]) <= 0)) return json(400,{error:`Nieprawidłowe pole: ${key}.`});
