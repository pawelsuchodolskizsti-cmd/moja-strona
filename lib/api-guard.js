...
function withApi(name, handler) {
  const adminEndpoints = ['admin-history','admin-records','admin-volunteers','admin-institutions','admin-results','admin-question-catalog','admin-adjust-participant','test-mode','qr-proxy'];
  const methods = {
  ...
  return async event => {
    try {
      ...
      const audited = method==='POST' && isAdminAuthorized(event) && ['game-state','admin-adjust-participant','admin-question-catalog','admin-institutions','admin-volunteers','test-mode','admin-history'].includes(name);
