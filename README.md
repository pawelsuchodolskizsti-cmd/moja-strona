...
`public/regulamin/index.html` i `public/rodo/index.html` zawierają **wzory** przygotowane na podstawie dokumentów organizatora, dopasowane do obecnej gry. Formularz zapisu pokazuje krótką informację o danych i linki do obu pełnych dokumentów. Informacja i linki są widoczne tylko w formularzu logowania uczestnika, przy akceptacji regulaminu. Regulamin zawiera wyłącznie zasady gry i załącznik dotyczący udziału osoby niepełnoletniej. Krótka informacja RODO dotyczy tylko danych uczestników gry. Treść zgody CRM i kontaktu pozostaje wyłącznie w `/komunikacja/`; z pozostałych dokumentów usunięto jej kopie i notatki dla organizatora. Pola do uzupełnienia pozostają oznaczone. Osobne, domyślnie niezaznaczone pole umożliwia testowy zapis na kontakt Fundacji; zasady są na `/komunikacja/`. Rejestr zapisuje cztery dane kontaktowe, wersję i treść oświadczenia, datę, deklarację wieku oraz status. Nie ma wysyłki ani integracji z zewnętrznym CRM. Wzór wymaga uzupełnienia i ponownego zebrania zgód przed wysyłką. Akceptacja regulaminu pozostaje jedynym wymaganym polem; udostępnienie informacji RODO nie jest dodatkową zgodą na przetwarzanie. Przed użyciem wersji ostatecznej organizator uzupełnia dane, okresy, warunki udziału i nagrody oraz zatwierdza treść. Wzory mają oznaczenie `2026-09-13-wzor-3`; ten identyfikator dokumentu nie jest obecnie zapisywany na koncie uczestnika.
...
Dobrowolny kontakt Fundacji: `/admin/komunikacja/` udostępnia chronioną listę po 50 zapisów i wycofanie zgody, odnotowane w logach admina. Uczestnik może wycofać zapis na `/komunikacja/#rezygnacja` kluczem zapisanym w localStorage (`foundationContactManageToken`); serwer przechowuje tylko skrót klucza. Zapis uczestnika i oświadczenia jest jedną transakcją. Tabela `communication_consents` nie jest kasowana przy resecie rundy. Ponowna próba tego samego zapisu nie odwołuje rezygnacji. Brak automatycznej retencji; organizator musi uzupełnić okresy i wdrożyć usuwanie. `wzór@email.com` jest wyłącznie przykładem, nie adresem do wysyłki.
